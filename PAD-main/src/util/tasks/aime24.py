...
class AIME24TaskUtils(BaseTaskUtils):
    """
    Utility class for handling AIME 2024 dataset tasks.
    ...
    def load_dataset(base_path=None):
        # Load AIME24 dataset from HuggingFace
    ...
    def evaluate(sequence_or_text, reference_txt, sample, start_idx=0, tokenizer=None, polish_reference=True):
        ...
        # Get the gold answer
        if polish_reference:
            gold_answer = AIME24TaskUtils.polish_ground_truth(sample["solution"])
        else:
        ...
        # print(f"Response Text: {sequence_txt}")
...
Step-by-Step Answer:"""
    ...
    def check_soundness(sample, response_text):
        raise NotImplementedError("Soundness check not implemented for AIME24.")