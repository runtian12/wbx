...
def maybe_init_dist() -> Optional[int]:
    ...
    print(f"Initializing distributed training with rank {rank} and world size {world_size}")
...
def _apply_tp_ffn(mlp: FeedForward) -> None:
    ...
    def all_reduce_hook(_module, _input, output):
...
def _apply_tp_attn(attn: Attention) -> None:
    ...
    _dim = attn.head_dim * attn.n_head
    _apply_tp_linear(attn.wqkv, "colwise", [_dim, kv_size, kv_size])
