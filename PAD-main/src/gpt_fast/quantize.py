...
def quantize(
    ...
    with torch.device('meta'):
        model_type = detect_model_type(checkpoint_path.parent.name)
        print(f"Detected model type: {model_type} for model name: {checkpoint_path.parent.name}")
        model_cls = get_model_cls(model_type)
        model = model_cls.from_name(checkpoint_path.parent.name)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Quantize a model. Supports LLaMA, Qwen2, and Qwen3 models.')
    parser.add_argument('--checkpoint_path', type=Path, default=Path("checkpoints/meta-llama/Llama-2-7b-chat-hf/model.pth"), help='Path to the model checkpoint to be quantized. Supports LLaMA, Qwen2, and Qwen3 models.')
